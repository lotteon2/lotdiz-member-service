...
if ! kubectl get deployment member-deployment &> /dev/null; then
  echo "Create new kubernetes deployment resources..."
  kubectl create -f ./deployment-prod.yml
else
  echo "Apply new docker image "
  kubectl set image deployments/member-deployment member-service=$ECR_REGISTRY/$AWS_ECR_REPOSITORY:$IMAGE_TAG --record
fi
...
if ! kubectl get deployment member-deployment &> /dev/null; then
  ...
  if ! kubectl rollout status deployment/member-deployment --watch=false | grep -q "successfully rolled out"; then
    echo "Rolling update has failed. Initiating rollback..."

    kubectl rollout undo deployment/member-deployment
    kubectl rollout status deployment/member-deployment --watch=true
    ...
    if ! kubectl get service member-service &> /dev/null; then
