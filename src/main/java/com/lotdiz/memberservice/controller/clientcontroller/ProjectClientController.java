...
public class ProjectClientController {
  ...
  public ResponseEntity<ResultDataResponse<MemberLikesInfoResponseDto>> calProjectLikesCnt(
    ...
    return ResponseEntity.ok()
        .body(
            new ResultDataResponse<>(
                String.valueOf(HttpStatus.OK.value()), HttpStatus.OK.name(), "성공", likesInfo));
  }
  ...
  public ResponseEntity<ResultDataResponse<MemberNameResponseDto>> getMembersName(
      @RequestBody List<Long> memberIds) {
    return ResponseEntity.ok()
        .body(
            ResultDataResponse.<MemberNameResponseDto>builder()
                .code(String.valueOf(HttpStatus.OK))
                .data(memberService.getMemberNames(memberIds))
                .detail("사용자 이름 조회")
                .build());
  }
}
